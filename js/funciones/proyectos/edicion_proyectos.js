...
function traer_procesos(){
    ...
    $.ajax({
        async:false,    
        cache:false,
        url: url + 'proyectos/traer_procesos/' +idProyecto,
        type: 'POST',
        timeout: 10000,
...
function traer_subprocesos(){
    ...
    $.ajax({
        async:false,    
        cache:false,
        url: url + 'proyectos/traer_subprocesos/'+ idProyecto,
        type: 'POST',
        timeout: 10000,
...
function preregistrar_documento(idDocumento, valor, tipo_documento){
    ...
    $.post( url + "proyectos/preregistrar",
    { 
        ...
        if (data.respuesta == 1){
            ...
            $("#observacion"+idDocumento).removeAttr("disabled");
            $("#noPreregistrados-"+data.idProceso).html(data.str_proceso);
            $("#noPreregistrados-subproceso-"+data.idSubProceso).html(data.str_subproceso);
        }else {
